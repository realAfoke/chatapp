...
class ChatConsumer(AsyncWebsocketConsumer):       
    ...
    async def receive(self, text_data = None, bytes_data = None):
        ...
        if self.conversation:
            ...
            if data.get('status') == 'received':
                msg_status= await database_sync_to_async(lambda:self.conversation.mssgconversation_reciept.filter(message_id=data.get('msgId')).first())()
                msg_status.status='delivered'
                await database_sync_to_async(msg_status.save)(update_fields=['status'])
    ...
    def mark_as_read(self,data):
        self.conversation.last_read_msg_id_id=data.get('lastReadMsgId')
        self.conversation.save(update_fields=['last_read_msg_id'])


    @database_sync_to_async
    def add_reaction_to_message(self,data):
        self.conversation.last_interaction='reaction'
        self.conversation.save(update_fields=['last_interaction'])
        MessageReaction.objects.create(user=self.current_user,conversation=self.conversation,message_id=data['msgId'],reaction=data['reaction'])
    ...
    def save_mssg(self,data):
        self.conversation.last_interaction='text'
        self.conversation.save(update_fields=['last_interaction'])
    ...
    def register_mssg_receipt(self,stats):
        if self.conversation.chat_type == 'group':
            return
        MessageReciept.objects.create(**stats)
